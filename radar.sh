...
WRITE_TO_SYSTEM_LOG='false'
# Define a tag for logging to system.log
LOGGING_TAG='com.expedia.radar'
...
# Default distribution type and ID. Attempt to set if no servers are pingable.
DEFAULT_DIST_TYPE='distribution_point'
DEFAULT_DIST_ID='7'
DEFAULT_DIST_PING='10000'
...
INTERNET_PORT='80'
CORP_URL='stash.sea.corp.expecn.com'
CORP_PORT='7999'
...
function Main () {
	...
	# Verify on the corporate network if not public, otherwise exit out.
	if [ "${JSS_IS_PUBLIC}" == 'false' ] && ! check_corp; then
		exit 0
	fi
...
function run_radar_distribution () {
	...
	# If there is a default distribution point, add that in with a really high ping value.
	# This ensures it is only picked if there are no other servers available.
	if [ -n "${DEFAULT_DIST_TYPE}" ] && [ -n "${DEFAULT_DIST_ID}" ] && [ -n "${DEFAULT_DIST_PING}" ]; then
		printf '%s %s %s\n' "${DEFAULT_DIST_PING}" "${DEFAULT_DIST_TYPE}" "${DEFAULT_DIST_ID}" >> "${tempFile}"
	fi
